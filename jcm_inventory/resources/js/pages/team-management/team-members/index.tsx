...
export default function TeamMembersIndex({
  ...
  const [roleId, setRoleId] = useState(filters.product_user_type_id ?? "");
  ...
  const [actionTarget, setActionTarget] = useState<MemberActionTarget | null>(
    null,
  );

  const memberForm = useForm<MemberFormData>(emptyMemberForm());

  const passwordForm = useForm<PasswordFormData>(emptyPasswordForm());

  useEffect(() => {
    setSearch(filters.search ?? "");
    setStatus(filters.status ?? "");
    setRoleId(filters.product_user_type_id ?? "");
  ...
  function openEditDialog(member: TeamMember): void {
    ...
    memberForm.setData({
      name: member.name,
      email: member.email,
      product_user_type_id: String(member.role.id),
  ...
  function handleMemberDialogChange(open: boolean): void {
    if (open) {
      setIsMemberDialogOpen(true);
      return;
    }
  ...
  function submitMember(event: FormEvent<HTMLFormElement>): void {
    event.preventDefault();

    if (editingMember) {
      memberForm.put(`/team/members/${editingMember.id}`, {
        preserveScroll: true,
  ...
  function openResetPasswordDialog(member: TeamMember): void {
  ...
  function handleResetPasswordDialogChange(open: boolean): void {
    if (open) {
      return;
    }
  ...
  function submitResetPassword(event: FormEvent<HTMLFormElement>): void {
    event.preventDefault();
    ...
    passwordForm.post(`/team/members/${resettingMember.id}/reset-password`, {
      preserveScroll: true,
  ...
  function requestStatusAction(member: TeamMember): void {
    setActionTarget({
      member,
  ...
  function requestRemoveMember(member: TeamMember): void {
    setActionTarget({
      member,
      action: "remove",
    });
  ...
  function executeMemberAction(): void {
    if (!actionTarget || processingMemberId !== null) {
      return;
    }
    ...
    const options = {
      ...
      onFinish: () => setProcessingMemberId(null),
    };

    if (action === "remove") {
      router.delete(`/team/members/${member.id}`, options);
  ...
  const setupReady = roles.length > 0 && branches.length > 0;
  ...
  const managerShare =
    summary.total > 0
      ? Math.round((summary.managers / summary.total) * 100)
      : 0;

  const hasActiveFilters = Boolean(
    search.trim() || status || roleId || branchId,
  );
  ...
  const actionDialog = getMemberActionDialog(actionTarget);
      ...
      cell: (member) => (
        <div className="space-y-2">
          ...
          <p className="text-[9px] text-muted-foreground">
            {member.role.code.toLowerCase() === "manager"
        ...
        return (
          <ActionGroup>
            ...
            <IconButton
              ...
              disabled={isProcessing}
              onClick={() => requestStatusAction(member)}
              className={cn(
                member.is_active
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-0.5 text-[10px] text-muted-foreground">
                  Manage account readiness, role assignments, branch coverage,
                  and team access.
                </p>
              </div>
            </div>
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-2 text-lg font-semibold">
                    {summary.active} active accounts
                  </p>

                  <p className="mt-1 line-clamp-2 text-[10px] leading-4 text-muted-foreground">
                    Manager and staff accounts currently available for inventory
                    operations.
                  </p>
                </div>
                ...
                <div className="flex flex-wrap items-end justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-[10px] text-muted-foreground">
                      Current workforce and branch assignment coverage.
                    </p>
                  </div>

                  <p className="text-[10px] font-medium text-muted-foreground">
                    {summary.total} total account
                    {summary.total === 1 ? "" : "s"}
                  </p>
                </div>
              ...
              <div className="p-4">
                ...
                <div className="mt-3 grid gap-2 md:grid-cols-3">
                  <TeamSignal
                    ...
                    tone={summary.inactive === 0 ? "emerald" : "amber"}
                  />
                  ...
                  <TeamSignal
                    ...
                    title={setupReady ? "Creation ready" : "Setup incomplete"}
                    description={
                      setupReady
        ...
        >
          ...
          >
            ...
            <Select
              ...
              onValueChange={(value) =>
                setRoleId(value === ALL_VALUE ? "" : value)
              }
            >
              <SelectTrigger className="h-10 w-full text-sm">
          ...
          <AppPagination pagination={members} itemLabel="team members" />
        </SectionCard>
      </PageContainer>
      ...
      <FormDialog
        ...
        submitText={editingMember ? "Save Changes" : "Create Account"}
        processingText={
          editingMember ? "Saving Changes..." : "Creating Account..."
        }
        maxWidth="max-w-3xl"
      >
        <FormSection
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            >
              <Input
                ...
                onChange={(event) =>
                  memberForm.setData("name", event.target.value)
                }
            ...
            >
              <Input
                ...
                disabled={memberForm.processing}
                onChange={(event) =>
                  memberForm.setData("email", event.target.value)
                }
        ...
        >
          <div className="grid gap-4 md:grid-cols-2">
            <FormField
              ...
              label="Team Role"
              error={memberForm.errors.product_user_type_id}
              required
            >
              <Select
                value={memberForm.data.product_user_type_id || NONE_VALUE}
                disabled={memberForm.processing}
                onValueChange={(value) =>
                  memberForm.setData(
                    "product_user_type_id",
              ...
              >
                <SelectTrigger id="product_user_type_id" className="w-full">
              ...
              <RoleDescription
                role={roles.find(
                  (role) =>
                    String(role.id) === memberForm.data.product_user_type_id,
                )}
              />
            </FormField>
            ...
            <FormField
              ...
              error={memberForm.errors.branch_id}
              required
            >
              <Select
                value={memberForm.data.branch_id || NONE_VALUE}
                disabled={memberForm.processing}
                onValueChange={(value) =>
                  memberForm.setData(
                    "branch_id",
        ...
        <FormSection
          title={editingMember ? "Credential Update" : "Temporary Credentials"}
          description={
            editingMember
        ...
        >
          <div className="grid gap-4 md:grid-cols-2">
            <FormField
              ...
              error={memberForm.errors.password}
            ...
            >
              <PasswordField
                ...
                onChange={(value) => memberForm.setData("password", value)}
              />
            </FormField>
            ...
            <FormField
              ...
              error={memberForm.errors.password_confirmation}
              required={!editingMember}
            >
              <PasswordField
                id="member_password_confirmation"
                ...
                onChange={(value) =>
                  memberForm.setData("password_confirmation", value)
                }
              />
            </FormField>
          </div>
          ...
          <div className="grid gap-4">
            ...
            >
              <PasswordField
                ...
                autoComplete="new-password"
                onToggle={() =>
                  setShowResetPasswordConfirmation((current) => !current)
                }
                onChange={(value) =>
      ...
      <ConfirmDialog
        ...
        onOpenChange={(open) => {
          if (!open && processingMemberId === null) {
        ...
        processing={processingMemberId !== null}
...
function MemberRoleBadge({ role }: { role: TeamMemberRole }) {
...
function RoleDescription({ role }: { role: RoleOption | undefined }) {
  if (!role?.description) {
    return null;
  }
...
function getMemberActionDialog(target: MemberActionTarget | null): {
  title: string;
  description: string;
  confirmText: string;
  destructive: boolean;
  ...
  if (!target) {
    return {
      ...
      description: "Confirm the requested account action.",
