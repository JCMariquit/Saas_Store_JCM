...
export type ThemePreset = 'jcm-dark' | 'ocean-enterprise' | 'violet-command' | 'amber-operations' | 'slate-minimal';
...
const isAppearance = (value: string | null): value is Appearance => value === 'light' || value === 'dark';

const isThemePreset = (value: string | null): value is ThemePreset => value !== null && Object.prototype.hasOwnProperty.call(THEME_PRESETS, value);
...
const getStoredAppearance = (): Appearance => {
    ...
    // Old installations may still have "system" saved.
    // Since System mode has been removed, fall back to the default Dark mode.
...
export function useAppearance() {
    ...
    useEffect(() => {
        ...
        const handleStorage = (event: StorageEvent) => {
            if (event.key === null || event.key === APPEARANCE_STORAGE_KEY || event.key === THEME_STORAGE_KEY) {
                syncFromStorage();
            }
        };
